...
class PurchaseRepositoryTest {
    ...
    void createPurchaseTest() {
        ...
        // 영속성 컨텍스트를 초기화하면 SELECT문을 볼 수 있다. 테스트 중이니 로그보려고 하는것(안하는게좋음)
        em.flush();
        em.clear();

        //then // 조회 / 최적화를 위해 select 실행안됨
    ...
    void findPurchaseListTest() {
        ...
        //then
        User user = userRepository.findById(user1.getId()).orElseThrow(); // 유저 가져옴
        List<Purchase> purchases = user.getPurchaseList(); // 그 유저의 구매목록 가져옴
    ...
    void cascadeRemoveTest() {
        ...
        List<Purchase> purchases = user.getPurchaseList(); // 구매목록에 2개

        System.out.println("\n\nuser1's purchases = " + purchases + "\n\n");
        System.out.println("\n\nall of purchases = " + purchaseRepository.findAll() + "\n\n"); // 총 구매기 록은 3개

        userRepository.delete(user); // 유저1 탈퇴(구매기록 2개 먼저 지우고 회원 지움)

        em.flush();
        em.clear();
        //when

        List<Purchase> purchaseList = purchaseRepository.findAll(); // 구매기록 1개 남아있어야함
